...
@pytest.fixture
def chrom_sizes_dict():
    return {"chr1": 1000000, "chr2": 800000}
...
class TestWriteBigwig:
    def test_write_and_read_back(self, tmp_path, chrom_sizes_dict):
        ...
        write_bigwig(scores, "chr1", 5000, chrom_sizes_dict, output)
    ...
    def test_larger_array(self, tmp_path, chrom_sizes_dict):
        """Write a 1000-element score array."""
        scores = np.random.default_rng(42).uniform(0, 100, size=1000)
        output = str(tmp_path / "test.bw")

        write_bigwig(scores, "chr1", 10000, chrom_sizes_dict, output)
    ...
    def test_unknown_chrom_raises(self, tmp_path, chrom_sizes_dict):
        scores = np.array([50.0])
        output = str(tmp_path / "test.bw")
        with pytest.raises(ValueError, match="not found"):
            write_bigwig(scores, "chrZ", 0, chrom_sizes_dict, output)

    def test_empty_scores(self, tmp_path, chrom_sizes_dict):
        """Empty score array produces a valid (empty) BigWig."""
        scores = np.array([], dtype=np.float64)
        output = str(tmp_path / "test.bw")
        write_bigwig(scores, "chr1", 0, chrom_sizes_dict, output)
