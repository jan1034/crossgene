...
def _make_hit(q_start, q_end, identity=0.9, bitscore=100.0, **kwargs) -> AlignmentHit:
    defaults = dict(
...
class TestWriteTsv:
    ...
    def test_single_hit(self, tmp_path):
        output = str(tmp_path / "test.tsv")
        hits = [_make_hit(1000, 1050, identity=0.92, bitscore=180.0)]
    ...
    def test_sorting(self, tmp_path):
        ...
        output = str(tmp_path / "test.tsv")
        hits = [
            _make_hit(2000, 2050, bitscore=50.0),
            _make_hit(1000, 1050, bitscore=80.0),
            _make_hit(1000, 1050, bitscore=100.0),
            _make_hit(2000, 2050, bitscore=90.0),
        ]
        write_tsv(hits, output)
